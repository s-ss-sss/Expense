	...
	<main class="u-mb-32">
		<section>
			...
			{if ! empty($types)}
				...
				<form action="/expense/admin/type/" method="POST">
									...
									<tr class="table__tr">
										<td class="table__td table__td--checkbox">
											<label class="table__checkbox-label">
												<input type="checkbox" name="type[]" value="{$type.id}" class="table__checkbox js-delete-check">
											</label>
										</td>
										<td class="table__td">{$type.type_name}</td>
										<td class="table__td table__td--action">
											<a href="/expense/admin/type/edit/{$type.id}/" class="button button--sub">修正する</a>
										</td>
									</tr>
								{/foreach}
							</tbody>
			...
			{else}
				<p class="course-list__no-item">種別は登録されていません</p>
			...
			{* 並び替え *}
			<form action="/expense/admin/type/" method="POST" class="js-sort-form" style="display: none;">
				<input type="hidden" name="state" value="sort">
			</form>

			{* 登録 *}
			<form action="/expense/admin/type/" method="POST" class="js-entry-form" style="display: none;">
